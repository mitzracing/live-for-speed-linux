...
deb_one="$TMP_ROOT/one/live-for-speed-linux_${VERSION}-1_amd64.deb"
deb_two="$TMP_ROOT/two/live-for-speed-linux_${VERSION}-1_amd64.deb"
[[ -f "$deb_one" && -f "$deb_two" ]]
...
if grep -Eq '(^|, )(wine32|[^, ]+:i386)([ ,]|$)' <<<"$depends"; then
  printf 'Debian package incorrectly requires an i386 Unix library stack for pure WoW64\n' >&2
  exit 1
fi
