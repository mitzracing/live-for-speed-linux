...
LFS 0.8C24 keeps the same 8C23 marker as its predecessor. Wrapper 0.3.3 therefore handles this one audited transition explicitly: an exact recorded C23 baseline is upgraded through the pinned C24 installer while complete player-owned paths and metadata are preserved. If the official updater already produced C24 files, setup accepts them without a second game download only when every protected entry is exact audited C24 content or an unchanged, previously trusted C23-only entry. Unknown drift still stops before mutation.
