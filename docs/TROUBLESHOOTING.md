...
LFS 0.8C24 is a special audited case because its executable identifies C24 but the protected tree retains the unchanged `8C23.txt` marker. Do not create a marker manually. Install wrapper 0.3.3 and run `lfs-linux install`. An exact recorded C23 baseline downloads and stages the pinned C24 installer while preserving complete player-owned roots, downloaded DDS skins, and metadata. If the official updater already produced C24, setup skips the second game download only when every protected entry matches the audited C24 target or an unchanged, previously trusted C23-only entry. Any unknown protected file or directory, special filesystem entry, or changed target/predecessor entry still fails closed.

## Upgrade stops before changing the game

Wrapper 0.3.3 upgrades a complete packaged LFS 0.8C20 payload or a complete content-addressed local LFS 0.8C23 baseline to packaged 0.8C24. Packaged migration uses the C20 stock and seed manifests to preserve changed player files. Local C23 migration preserves dedicated player-owned paths with `cp -a`; downloaded `data/skins_dds` files are merged without replacing audited C24 stock. Directory, file, link, ownership, mode, timestamp, xattr, and ACL-backed metadata remain intact. A player file that collides with a stock path, and every verified C23-only protected entry removed from the active C24 tree, is retained by content hash under `~/.local/share/lfs-linux/migration-conflicts/`. Same-marker adoption restores the whole predecessor-only set if any entry changes before package metadata commits. Interrupted-swap recovery validates its backup before touching the current tree; an incomplete current tree is retained under `migration-conflicts/interrupted-current-game-tree` rather than deleted. A valid locally recorded newer game is preserved instead of replaced. Unknown or unrecorded protected-file drift stops before mutation.
